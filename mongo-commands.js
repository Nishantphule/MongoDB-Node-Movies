...
db.movies.insertMany([
  ...
  // to get all data inside collection
db.movies.find({})
...
// inclusion
db.movies.find({}, { title:1 , rating: 1}).pretty()

// exclusion
db.movies.find({}, { description:0 , pic: 0}).pretty()

// cant mix both 1 and 0 - hence i will not work
db.movies.find({}, { title:1 , pic: 0}).pretty()  

// both can mix only when 0 is for _id - only exception
db.movies.find({}, {_id:0, title:1 , rating: 1}).pretty() 
db.movies.find({}, {_id:0, title:1 , language: 1}).pretty()

// sorting
db.movies.find({}).sort({ rating: 1}).pretty()

db.movies.find({}, {_id:0, title:1 , rating: 1}).sort({ rating: 1}).pretty()

db.movies.find({}).sort({ rating: -1})

db.movies.find({}, {_id:0,title:1,rating:1}).sort({ rating: 1})

db.movies.find({}, {_id:0,title:1,rating:1}).sort({ rating: 1, title:1}).pretty()


// limit
db.movies.find({}, {_id:0,title:1,rating:1}).sort({ rating: -1, title:1}).limit(3).pretty()

// skip
db.movies.find({}, {_id:0,title:1,rating:1}).sort({ rating: -1, title:1}).limit(3).skip(3).pretty()
...
db.movies.aggregate([
  { $match: { rating :"6.3"}},
  { $group: { _id: "$title", totalUrgentQuantity: { $sum: "$rating"}}}
])



// update all movies to english 

db.movies.updateMany({}, 
{ $set : { language: "english"}})

db.movies.find({ title :{ $in :["Pushpa:The Rise - Part 1","K.G.F: Chapter 1"]}})

db.movies.updateMany({ title :{ $in :["Pushpa:The Rise - Part 1","K.G.F: Chapter 1"]}}, 
{ $set : { language: "Telugu"}})
    
db.movies.updateOne({ title :{ $in :["Bachchhan Paandey"]}}, 
{ $set : { language: "Hindi"}})
